...
case "end" :     
	...
	for(variables.myRow = 1; variables.myRow <= attributes.query.recordcount; variables.myRow++)	{
		...
		QuerySetCell(thisTag.Parent.thisTag.qryOption, "disabled", 	attributes?.disabled == true);
		...
		QuerySetCell(thisTag.Parent.thisTag.qryOption, "selected",	(attributes?.selected == true || attributes.selectedValue.contains(variables.myValue)));
